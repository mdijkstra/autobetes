function dbHandler(shortName, version, displayName, maxSize) {
	...
	*/
	...
	var CREATE_EVENT = 'CREATE TABLE IF NOT EXISTS Event(cId INTEGER PRIMARY KEY AUTOINCREMENT, sId INTEGER, eventType TEXT NOT NULL, name TEXT NOT NULL, beenSent INTEGER DEFAULT 0, deleted INTEGER DEFAULT 0, lastchanged INTEGER NOT NULL)';
	var CREATE_EVENT_INSTANCE = 'CREATE TABLE IF NOT EXISTS EventInstance ( cId INTEGER PRIMARY KEY AUTOINCREMENT, sId INTEGER, Dtype TEXT DEFAULT NULL, beginTime INTEGER NOT NULL, cEvent INTEGER NOT NULL, sEvent INTEGER, beenSent INTEGER DEFAULT 0, deleted INTEGER DEFAULT 0, lastchanged INTEGER NOT NULL, CONSTRAINT FK_EventInstance_Event FOREIGN KEY (cEvent) REFERENCES Event (cId))';
	...
	var UPDATE_EVENT_INSTANCE = 'UPDATE EventInstance SET beginTime = ?, lastchanged=?, beenSent = 0 WHERE cId=?';
	var SERVER_UPDATE_EVENT = 'UPDATE Event SET sId=?, name=?, eventType =?, lastchanged=?, deleted=? WHERE cId =?';
	var SERVER_UPDATE_INSTANCE = 'UPDATE EventInstance SET sId = ?, sEvent = ?, beginTime = ?, deleted=?, lastchanged=?, beenSent = 0 WHERE cId=?';
	...
	var DELETE_INSTANCE = 'UPDATE EventInstance SET deleted = 1, lastchanged=?, beenSent = 0 WHERE cId = ?';
	...
	var SELECT_CURRENT_EVENT_INSTANCES = 'SELECT e.beginTime, a.intensity, e.cId, ev.name, a.endTime from Event ev join EventInstance e on ev.cId = e.cEvent join ActivityEventInstance a on a.cId = e.cId WHERE e.deleted = 0 AND a.endTime IS NULL ORDER BY e.beginTime DESC;';
	var SELECT_FOOD_EVENT_INSTANCES = 'SELECT * from Event ev join EventInstance e on ev.cId = e.cEvent join FoodEventInstance f on e.cId = f.cId where e.deleted = 0 ORDER BY e.beginTime DESC;';
	var SELECT_ACTIVITY_EVENT_INSTANCES = 'SELECT * from Event ev join EventInstance e on ev.cId = e.cEvent join ActivityEventInstance a on e.cId = a.cId where e.deleted = 0  AND a.endTime IS NOT NULL ORDER BY e.beginTime DESC;';
	...
	var ADD_ACTIVITY_WITH_ENDTIME = 'INSERT INTO ActivityEventInstance(cId, intensity, endTime) VALUES (?,?,?)';
	...
	db.transaction(function(tx) {
		...
		tx.executeSql( CREATE_LAST_UPDATE, [], nullHandler, errorHandler);
		...
		//insert timestamp = 0 if in last_update table if table contains no values
		
		
		tx.executeSql( SELECT_LAST_UPDATE_TIMESTAMP, [], function(transaction,result){
			if(result.rows.length === 0){
				//table contains no value
				//populate table
				tx.executeSql( ADD_LAST_UPDATE_TIMESTAMP, [], nullHandler, errorHandler);
			}
			else{
		...
		tx.executeSql( SELECT_USER_INFO, [], function(transaction,result){
			if(result.rows.length === 0){
				//table contains no value
...
/*
	...
	*/
	...
	function serverUpdateEntity(entityType, entity, row){//sId, sEvent, type, amount,beginTime,endTime, deleted, timeStampLastChanged, cId
		...
		if(entityType === EVENT){
			console.log("update event:" + JSON.stringify(entity));
			...
			console.log("update instance:" + JSON.stringify(entity));
		...
		if (entity.intensity) {
			executeQuery(UPDATE_ACTIVITY, [entity.intensity, entity.endTime, row.cId], nullHandler);
			
		} else {
			executeQuery(UPDATE_FOOD, [entity.amount, row.cId], nullHandler);
	...
	function addEventInstance(quantity, beginTime, cId, eventType) {
		...
		getEntityWithCId(cId, EVENT, function(transaction, result){
			...
			var addFoodFunction = function(transaction, result){
				executeQuery(ADD_FOOD, [result.insertId, quantity],synchronise);
			};
			var addActivityFunction = function(transaction, result){
				executeQuery(ADD_ACTIVITY, [result.insertId, quantity],synchronise);
			};
			if(eventType === FOOD){
	...
	function serverAddEntity(entityType,entity){ //sId, sEvent, type, beginTime, endTime, intensity, deleted, lastchanged
		...
		var getCEVENTFunction = function(transaction, result){
			if(result.rows.length === 0){
				...
				if (entity.intensity) {
					var addActivityFunction = function(transaction, result){
						executeQuery(ADD_ACTIVITY_WITH_ENDTIME, [result.insertId, entity.intensity, entity.endTime], nullHandler);
					};
					...
					var addFoodFunction = function(transaction, result){
						executeQuery(ADD_FOOD, [result.insertId, entity.amount],nullHandler);
					};
	...
	function updateUser(email, password, callback){
		executeQuery(EDIT_USER, [email, password], callback);
	}
